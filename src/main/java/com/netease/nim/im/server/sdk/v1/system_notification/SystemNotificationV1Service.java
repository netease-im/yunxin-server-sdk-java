...
public class SystemNotificationV1Service implements ISystemNotificationV1Service {
    ...
    public Result<SendAttachMsgResponseV1> sendAttachMsg(SendAttachMsgRequestV1 request) throws YunxinSdkException {
        ...
        // 处理JSON数组类型的参数
        if (request.getForcePushList() != null) {
            // 确保列表已经是JSON字符串格式
            if (!request.getForcePushList().startsWith("[")) {
                paramMap.put("forcePushList", JSONArray.toJSONString(JSON.parseArray(request.getForcePushList())));
            }
        }
