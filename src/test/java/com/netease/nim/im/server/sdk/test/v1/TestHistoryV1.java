...
public class TestHistoryV1 {
        ...
        try {
            ...
            if (result.isSuccess() && result.getResponse() != null && result.getResponse().getMsgs() != null) {
                ...
                for (Message message : messages) {
                    ...
                    System.out.println("Session message msgid (Long): " + message.getMsgid() + ", msgidclient: " + message.getMsgidclient());
    ...
    private static void queryTeamHistoryMessage(Long tid, String accid) {
        try {
            ...
            if (result.isSuccess() && result.getResponse() != null && result.getResponse().getMsgs() != null) {
                List<Message> messages = result.getResponse().getMsgs();
                System.out.println("Retrieved " + messages.size() + " team messages");
                for (Message message : messages) {
                    System.out.println("Team message type: " + message.getType() + ", from: " + message.getFrom());
                    System.out.println("Team message msgid (Long): " + message.getMsgid() + ", msgidclient: " + message.getMsgidclient());
                    System.out.println("Team message body: " + JSON.toJSONString(message.getBody()));
                }
            }
    ...
    private static QueryChatroomHistoryMessageResponseV1 queryChatroomHistoryMessage(Long roomid, String accid) {
        try {
            ...
            // 输出消息详情，用于验证消息模型
            if (result.isSuccess() && result.getResponse() != null && result.getResponse().getMsgs() != null) {
                List<ChatroomMessage> messages = result.getResponse().getMsgs();
                ...
                for (ChatroomMessage message : messages) {
                    ...
                    System.out.println("Chatroom message msgid (String): " + message.getMsgid() + " (no msgidclient)");
    ...
    private static CreateChatroomResponseV1 createChatroom(String creator) {
        try {
            ...
            request.setName("TestChatRoom-" + System.currentTimeMillis());
        ...
        try {
            ...
            if (result.isSuccess() && result.getResponse() != null && result.getResponse().getMsgs() != null) {
                ...
                for (Message message : messages) {
                    ...
                    System.out.println("客户端消息ID: " + message.getMsgidclient());
