...
import com.netease.nim.server.sdk.im.v2.message.request.*;
import com.netease.nim.server.sdk.im.v2.message.response.*;
...
public class TestMessageV2 {
    ...
    @Test
    public void testStreamMessage() throws YunxinSdkException {
        if (services == null) return;
        ...
        attachment.setText("This is a stream message.");
    ...
    private void testSearchByKeyword() throws YunxinSdkException {
        ...
        strings.add(searchKeyword);
        request.setKeywordList(strings);
        request.setLimit(20);
    ...
    private void testSearchWithMultipleCriteria() throws YunxinSdkException {
        ...
        objects.add(searchKeyword);
        request.setKeywordList(objects);
    ...
    @Test
    public void testqueryMessagesByPage() throws Exception {
        if (services == null) return;
    ...
    private void testBasicConversationQuery(String conversationId, long startTime, long endTime) throws YunxinSdkException {
        ...
        // Execute query
        Result<QueryMessagesByPageResponseV2> result = messageService.queryMessagesByPage(request);
    ...
    private void testConversationPagination(String conversationId, long startTime, long endTime) throws YunxinSdkException {
        ...
        if (result.getResponse().getHasMore() != null && result.getResponse().getHasMore()
            ...
            Result<QueryMessagesByPageResponseV2> nextResult = messageService.queryMessagesByPage(nextRequest);
    ...
    private void testConversationSortingOrder(String conversationId, long startTime, long endTime) throws YunxinSdkException {
        ...
        Result<QueryMessagesByPageResponseV2> ascResult = messageService.queryMessagesByPage(ascRequest);
        ...
        Result<QueryMessagesByPageResponseV2> descResult = messageService.queryMessagesByPage(descRequest);
    ...
    public static BatchSendP2PMessageRequestV2 createCustomMessage(String senderId, List<String> receiverIds, 
                                                                  Integer subType, Map<?, ?> attachment) {
        BatchSendP2PMessageRequestV2 request = new BatchSendP2PMessageRequestV2();
        request.setSenderId(senderId);
        request.setReceiverIds(receiverIds);
    ...
    public void testSendTeamReadReceipt() throws YunxinSdkException {
        ...
        Long teamId = Long.valueOf(createAdvancedTeam(teamName));
        if (teamId == null) {
    ...
    /**
     * Test sending a complex message with all configurations including thread config
     */
    @Test
    public void testSendComplexMessageWithThreadConfig() throws YunxinSdkException {
        if (services == null) return;
        
        System.out.println("\n==== Testing Complex Message with Thread Config ====");
        ...
        // Create message request
        SendMessageRequestV2 request = new SendMessageRequestV2();
        request.setSenderNoSense(false);
        request.setReceiverNoSense(false);

        // message
        SendMessageRequestV2.MessageBody message = new SendMessageRequestV2.MessageBody();
        message.setMessageType(0);
        message.setSubType(1);
        message.setText("{\"msg\":\"2025-07-18T16:48:23.120文本消息测试-24o753nuch\"}");

        Map<String, Object> attachment = new HashMap<>();
        attachment.put("msg", "2025-07-18T16:48:23.120文本消息测试-24o753nuch");
        message.setAttachment(attachment);

        request.setMessage(message);

        // thread_config
        SendMessageRequestV2.ThreadConfig threadConfig = new SendMessageRequestV2.ThreadConfig();

        SendMessageRequestV2.ThreadMessage threadRoot = new SendMessageRequestV2.ThreadMessage();
        threadRoot.setSenderId("qaz");
        threadRoot.setReceiverId("qaz1");
        threadRoot.setCreateTime(1752828500064L);
        threadRoot.setMessageServerId(14979609530916L);
        threadRoot.setMessageClientId("3b8e4009-0f6d-4406-8b92-9503b9209a34");
        threadConfig.setThreadRoot(threadRoot);

        SendMessageRequestV2.ThreadMessage threadReply = new SendMessageRequestV2.ThreadMessage();
        threadReply.setSenderId("qaz");
        threadReply.setReceiverId("qaz1");
        threadReply.setCreateTime(1752828500064L);
        threadReply.setMessageServerId(14979609530916L);
        threadReply.setMessageClientId("3b8e4009-0f6d-4406-8b92-9503b9209a34");
        threadConfig.setThreadReply(threadReply);

        request.setThreadConfig(threadConfig);

        // 只设置必要的会话ID
        request.setConversationId("qaz|1|qaz1");

        // 发送
        Result<SendMessageResponseV2> result = messageService.sendMessage(request);
        
        System.out.println("sendComplexMessageWithThreadConfig: " + result.getMsg());
        System.out.println("Response: " + JSON.toJSONString(result));
        ...
        Assert.assertEquals(Integer.valueOf(1), response.getConversationType());
        Assert.assertEquals(Integer.valueOf(0), response.getMessageType());
        Assert.assertNotNull(response.getMessageServerId());
        Assert.assertNotNull(response.getCreateTime());
        Assert.assertNotNull(response.getMessageClientId());
        ...
        System.out.println("MessageClientId: " + response.getMessageClientId());
